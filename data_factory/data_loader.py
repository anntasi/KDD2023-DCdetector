...
# English comment: Typing helpers.
    ...
    def __getitem__(self, index):
        ...
        if self.flag == "train":
            ...
            return np.float32(self.test[
                              index // self.step * self.win_size:index // self.step * self.win_size + self.win_size]), np.float32(
                self.test_labels[index // self.step * self.win_size:index // self.step * self.win_size + self.win_size])
class TNSSegLoader(Dataset):
    """
    English comment:
    Session-aware window loader for TNS PDU features.
    """

    def __init__(self, root_path, win_size, step=1, flag="train", drop_col_idx=5):
        self.flag = flag
        self.step = int(step)
        ...
        # ---- Load features ----
        feats_path = os.path.join(root_path, "pdu_features.npy")
        X = np.load(feats_path)  # (N, 9)

        # Drop byte_min column (0-based index = 4)
        X = np.delete(X, int(drop_col_idx), axis=1)  # -> (N, 8)

        X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)

        # Global standardization (fit once)
        self.scaler.fit(X)
        self.X_all = self.scaler.transform(X).astype(np.float32)
        self.N, self.D = self.X_all.shape

        # ---- Load sessions index ----
        idx_path = os.path.join(root_path, "sessions_index.csv")
        df = pd.read_csv(idx_path)

        if "conn_id" not in df.columns or "meta_row_indices" not in df.columns:
            raise ValueError("sessions_index.csv must have columns: conn_id, meta_row_indices")

        # Parse mapping: conn_id -> [row_idx0, row_idx1, ...]
        self.sessions: Dict[str, List[int]] = {}
        for _, r in df.iterrows():
            cid = str(r["conn_id"])
            s = str(r["meta_row_indices"]).strip()
            s = s.strip('"').strip("'").strip()
            rows = [] if s == "" else [int(x) for x in s.split(",") if x != ""]
            self.sessions[cid] = rows

        # ---- Build window index list (session-aware) ----
        # Each entry: (cid, start_pos) where start_pos is position within that session's row list
        self.win_index = []
        for cid, rows in self.sessions.items():
            T = len(rows)
            if T < self.win_size:
                continue
            for start in range(0, T - self.win_size + 1, self.step):
                self.win_index.append((cid, start))

        # Dummy labels to satisfy existing (X, Y) unpacking
        self._dummy_y = np.zeros((self.win_size, 1), dtype=np.float32)
        
        print("[TNS] X_all:", self.X_all.shape)
        print("[TNS] num_windows:", len(self.win_index))
        print("[TNS] first_window:", self.win_index[0], self.sessions[self.win_index[0][0]][0:self.win_size])
    ...
    def __getitem__(self, index):
        cid, start = self.win_index[index]
        rows = self.sessions[cid][start:start + self.win_size]
        Xw = self.X_all[rows]  # (win_size, D)
        return np.float32(Xw), self._dummy_y
...
def get_loader_segment(index, data_path, batch_size, win_size=100, step=100, mode='train', dataset='KDD'):
    if (dataset == 'SMD'):
        ...
        dataset = TNSSegLoader(data_path, win_size, 1, mode, drop_col_idx=5)
